...
def splitcmd(cmd) :
#expects a string with a starting letter and a number
#returns letter, number
	udlr = cmd[0]
	iters = int(cmd[1:])
	return udlr,iters
	
def placewireA(udlr_iters):
    udlr,iters = udlr_iters
    global xpos
    global ypos
    global gridA
    global wirelength
    ...
    for i in range(0,iters):
        if udlr == 'U':
            ...
            xpos += 1
        gridA.append((xpos,ypos))
        wirelength +=1
        gridADist.append(wirelength)


def placewireB(udlr_iters):
    udlr,iters = udlr_iters
    global xpos
    global ypos
    global gridB
...
for i in gridA:
    if (i in setB):
        indA = gridA.index(i)
        indB = gridB.index(i)
        distlist.append(gridADist[indA]+gridBDist[indB])

print("min dist:",min(distlist))